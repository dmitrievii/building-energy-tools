"""GeoSphere Austria native-monthly transport for ``klima-v2-1m``.

The monthly resource is a semantically independent provider dataset. It keeps
published monthly means, extrema, sums and event statistics at native calendar-
month resolution and never fabricates hourly or daily samples. Analysis routing
is owned by the shared canonical Climate Analyzer page architecture.
"""
...
def ensure_monthly_resource_registered() -> None:
    """Register the monthly resource without forcing it into hourly field specs."""
    ...
    geosphere.GEOSPHERE_RESOURCES[MONTHLY_RESOURCE_ID] = geosphere.GeoSphereResourceSpec(
        ...
        # The shared resource dataclass requires an integer cadence. Calendar-
        # month planning below never uses this nominal value for time stepping.
...
def monthly_parameter_mapping(metadata: Mapping[str, Any]) -> dict[str, geosphere.ProviderFieldSpec]:
    """Expose every live physical monthly parameter except quality flags."""
...
def monthly_metadata_bundle(metadata: Mapping[str, Any]) -> tuple:
    """Return the metadata bundle shape expected by the shared source UI."""
    ...
    return (
        ...
        {},  # no reliable per-parameter station validity windows in metadata
...
def month_count(start: pd.Timestamp, end: pd.Timestamp) -> int:
    """Return the inclusive calendar-month count for an arbitrary range."""
...
def _month_start(value: pd.Timestamp) -> pd.Timestamp:
    stamp = pd.Timestamp(value)
    return pd.Timestamp(year=int(stamp.year), month=int(stamp.month), day=1, tz=stamp.tz)
...
def plan_monthly_queries(
    ...
    *,
    ...
    """Split one monthly request on true calendar-month boundaries."""
...
GeoSphereMonthlyProgressCallback = Callable[[Mapping[str, object]], None]


def _emit_progress(
    callback: GeoSphereMonthlyProgressCallback | None,
    *,
    event: str,
    completed: int,
    total: int,
...
def fetch_monthly_provider_frame(
    *,
    ...
    progress_callback: GeoSphereMonthlyProgressCallback | None = None,
...
def monthly_coverage_table(data: pd.DataFrame) -> pd.DataFrame:
    """Return independent per-variable coverage; one field never masks another."""
