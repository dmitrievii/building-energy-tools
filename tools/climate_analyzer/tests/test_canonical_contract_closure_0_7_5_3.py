...
from epw_climate_analyzer.source_parity_contract_closure import (
    _monthly_semantics,
    monthly_catalogue_role,
    monthly_period_values,
    monthly_psychrometric_frame,
    monthly_psychrometric_profile,
)
...
class CanonicalContractClosureTests(unittest.TestCase):
    def test_native_monthly_mean_is_weighted_by_real_calendar_days(self) -> None:
        ...
        annual = monthly_period_values(frame, "dry_bulb_temperature_c", "Annual")
    ...
    def test_native_monthly_sum_and_extrema_keep_quantity_semantics(self) -> None:
        ...
        self.assertAlmostEqual(float(monthly_period_values(frame, "liquid_precipitation_depth_mm", "Annual").iloc[0]), 60.0)
        self.assertAlmostEqual(float(monthly_period_values(frame, "dry_bulb_temperature_min_c", "Annual").iloc[0]), -10.0)
        self.assertAlmostEqual(float(monthly_period_values(frame, "dry_bulb_temperature_max_c", "Annual").iloc[0]), 18.0)
    ...
    def test_native_monthly_seasonal_groups_real_calendar_seasons(self) -> None:
        ...
        frame = pd.DataFrame({"liquid_precipitation_depth_mm": [10.0, 20.0, 30.0, 40.0]}, index=index)
        seasonal = monthly_period_values(frame, "liquid_precipitation_depth_mm", "Seasonal")
    ...
    def test_native_monthly_direction_uses_weighted_circular_mean(self) -> None:
        ...
        annual = monthly_period_values(frame, column, "Annual")
        value = float(annual.iloc[0])
        self.assertTrue(value >= 350.0 or value <= 10.0)

    def test_unknown_provider_statistic_stays_monthly_only(self) -> None:
        column = "monthly__unknown__other__foo"
        self.assertIsNone(_monthly_semantics(column))
        frame = pd.DataFrame({column: [1.0, 2.0]}, index=pd.DatetimeIndex(["2025-01-01", "2025-02-01"]))
        with self.assertRaises(ValueError):
            monthly_period_values(frame, column, "Seasonal")
    ...
    def test_monthly_psychrometric_properties_are_representative_state_properties(self) -> None:
        ...
        out = monthly_psychrometric_frame(dataset, frame)
        for column in (
            "humidity_ratio_g_kg",
        ...
        self.assertIn("representative monthly states", out.attrs["canonical_monthly_psychrometric_semantics"])
    ...
    def test_monthly_psychrometric_profile_collapses_multiyear_data_to_twelve_calendar_points(self) -> None:
        ...
        profile = monthly_psychrometric_profile(frame, 100000.0)
    ...
    def test_catalogue_roles_distinguish_core_additional_and_other(self) -> None:
        core = SimpleNamespace(provider="tl_mittel", recommended=True, category="Temperature", annual_semantics="mean")
        additional = SimpleNamespace(provider="frost_days", recommended=False, category="Temperature", annual_semantics="sum")
        other = SimpleNamespace(provider="mystery", recommended=False, category="Other monthly parameters", annual_semantics=None)
        self.assertEqual(monthly_catalogue_role(core), "Core variable")
        self.assertEqual(monthly_catalogue_role(additional), "Additional statistic")
        self.assertEqual(monthly_catalogue_role(other), "Other provider parameter")
