...
async function filterStation(page, stationName) {
  ...
  while (performance.now() - started < 60_000) {
    ...
    const match = lastText.match(/Visible GeoSphere stations after filters:\s*([\d,]+)/);
    const count = match ? Number(match[1].replaceAll(',', '')) : null;
    if (count === 1 && lastText.includes(stationName)) return frame;
    await sleep(300);
  }
  throw new Error(`Station filter did not settle on ${stationName}.`);
}
...
const report = {
  schema: 'climate-analyzer-pr83-source-ui-smoke-v16',
...
try {
  ...
  await filterStation(page, fixture.station_name);
