...
async function selectDataset(page) {
  ...
  while (performance.now() - started < 60000) {
    ...
    const option = await visibleOption(page, frame, (v) => v.includes('1 month'));
...
async function selectStation(page, stationName, stationId) {
  ...
  while (performance.now() - started < 60000) {
    ...
    if (await button.count().catch(() => 0) && await button.isVisible().catch(() => false)) {
      try { await button.click({ timeout: 10000 }); return await appFrame(page, 'Measured variables to load', 60000); } catch {}
    }
    await sleep(250);
  }
...
async function waitMonthlySurface(page, timeoutMs = 60000) {
  ...
  while (performance.now() - started < timeoutMs) {
    ...
    const text = await bodyText(frame); last = text;
    const ready = text.includes('Selected resource: klima-v2-1m') && text.includes('Parameter set') && text.includes('Core variables') && text.includes('Core + additional statistics') && text.includes('All provider parameters') && text.includes('Select at least one measured GeoSphere variable to load.');
    if (ready) { stable += 1; if (stable >= 3) return frame; } else stable = 0;
    await sleep(300);
  }
...
async function selectParameterSet(page, name) {
  ...
  while (performance.now() - started < 45000) {
    ...
    if (await radio.count().catch(() => 0) && await radio.isVisible().catch(() => false)) {
      ...
      await radio.check({ force: true, timeout: 5000 }).catch(async () => frame.getByText(name, { exact: true }).last().click({ force: true, timeout: 5000 }).catch(() => {}));
    } else await frame.getByText(name, { exact: true }).last().click({ force: true, timeout: 5000 }).catch(() => {});
    await sleep(600);
...
async function positionEditor(page, side = 'left') {
  let current = await variableEditor(page);
  await current.editor.locator('.dvn-scroller').first().evaluate((node, side) => {
    node.scrollTop = 0; node.scrollLeft = side === 'right' ? node.scrollWidth : 0; node.dispatchEvent(new Event('scroll', { bubbles: true }));
  }, side).catch(() => {});
  await sleep(900);
  return variableEditor(page);
}
async function providerState(page, provider) {
  const right = await positionEditor(page, 'right');
  const rows = right.editor.locator('[role="grid"] tr[role="row"]');
  let aria = null;
  for (let i = 0; i < await rows.count(); i += 1) {
    const row = rows.nth(i); const texts = await row.locator('[role="gridcell"]').allTextContents().catch(() => []);
    if (texts.map((x) => x.trim()).includes(provider)) { aria = Number(await row.getAttribute('aria-rowindex')); break; }
  }
  if (!Number.isFinite(aria) || aria < 2) throw new Error(`Provider ${provider} not visible after exposing Provider column.`);
  const left = await positionEditor(page, 'left');
  const row = left.editor.locator(`[role="row"][aria-rowindex="${aria}"]`).first();
  if (!(await row.count().catch(() => 0))) throw new Error(`Provider row ${aria} disappeared for ${provider}.`);
  const load = String(await row.locator('[role="gridcell"][aria-colindex="1"]').first().textContent().catch(() => '')).trim().toLowerCase();
  return { ...left, load, ariaRowIndex: aria, dataIndex: aria - 2 };
}
async function navigateLoadCell(page, state, provider) {
  const expected = `glide-cell-0-${state.dataIndex}`;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const current = await positionEditor(page, 'left'); const box = await current.canvas.boundingBox().catch(() => null);
    if (!box) continue;
    ...
    await current.canvas.focus(); await page.keyboard.press('Control+Home'); await sleep(200);
    if (await current.editor.locator('[role="gridcell"][aria-selected="true"]').first().getAttribute('data-testid').catch(() => null) !== 'glide-cell-0-0') continue;
    for (let row = 0; row < state.dataIndex; row += 1) await page.keyboard.press('ArrowDown');
    await sleep(200);
    if (await current.editor.locator('[role="gridcell"][aria-selected="true"]').first().getAttribute('data-testid').catch(() => null) === expected) return;
  }
...
async function selectProvider(page, provider) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    await navigateLoadCell(page, state, provider); await page.keyboard.press('Space');
    const started = performance.now();
...
async function clickSidebar(page, label, expectedText) {
  const started = performance.now();
  while (performance.now() - started < 60000) {
    const frame = await appFrame(page, 'Climate Analyzer', 10000); const sidebar = frame.locator('section[data-testid="stSidebar"]');
    for (const c of [sidebar.getByText(label, { exact: true }).last(), sidebar.locator('label').filter({ hasText: label }).last()]) {
      if (!(await c.count().catch(() => 0)) || !(await c.isVisible().catch(() => false))) continue;
      try { await c.click({ force: true, timeout: 5000 }); return await appFrame(page, expectedText, 30000); } catch {}
    }
    await sleep(300);
  }
...
const report = { schema: 'climate-analyzer-monthly-selected-load-smoke-v3', target_url: TARGET_URL, fixture, checks: {}, page_errors: [], console_errors: [], success: false, error: null };
let browser; let page;
try {
  if (!fixture.success || String(fixture.station_id) !== '105') throw new Error('Monthly fixture must resolve station 105 successfully.');
  ...
  await chooseSource(page); await selectDataset(page); await selectStation(page, fixture.station_name, fixture.station_id);
  let frame = await waitMonthlySurface(page); let text = await bodyText(frame);
  ...
  if (await frame.getByRole('button', { name: 'Load measured GeoSphere interval', exact: true }).count().catch(() => 0)) throw new Error('Load button reachable with empty selection.');
  report.checks.initial_selection = 'zero'; report.checks.empty_selection_warning = true; report.checks.initial_core_view = true;
  await selectParameterSet(page, 'All provider parameters'); const selected = {};
  for (const provider of REQUIRED) { const state = await selectProvider(page, provider); selected[provider] = { load: state.load, row: state.ariaRowIndex }; }
  report.checks.selected_provider_parameters = selected;
  frame = await appFrame(page, 'Measured variables to load', 20000); text = await bodyText(frame);
  if (text.includes('GeoSphere load complete')) throw new Error('Checkbox edit triggered provider loading before mature load button.');
  report.checks.checkbox_edit_did_not_activate_dataset = true;
  await selectParameterSet(page, 'Core variables'); frame = await appFrame(page, 'Measured variables to load', 20000);
  if (!(await frame.getByRole('button', { name: 'Load measured GeoSphere interval', exact: true }).count().catch(() => 0))) throw new Error('Selection lost on All -> Core transition.');
  report.checks.core_all_core_load_button_preserved = true;
  await selectParameterSet(page, 'All provider parameters'); for (const provider of REQUIRED) { const state = await providerState(page, provider); if (state.load !== 'true') throw new Error(`Provider-keyed roundtrip lost ${provider}.`); }
  report.checks.provider_keyed_roundtrip = true;
  await selectParameterSet(page, 'Core variables'); frame = await appFrame(page, 'Measured variables to load', 20000);
  const loadButton = frame.getByRole('button', { name: 'Load measured GeoSphere interval', exact: true }).first(); if (!(await loadButton.count().catch(() => 0))) throw new Error('Mature GeoSphere load button missing.');
  await loadButton.click({ timeout: 15000 }); frame = await appFrame(page, 'Summary — Overview', 180000); text = await bodyText(frame);
  if (!text.includes('Climate overview')) throw new Error('Monthly canonical dataset did not activate Overview.'); report.checks.canonical_dataset_activated = true; report.checks.summary_overview = true;
  frame = await clickSidebar(page, 'Climate — Temperature', 'Temperature and extremes'); text = await bodyText(frame); if (!text.includes('Dew-point temperature')) throw new Error('Dew-point capability missing.'); report.checks.temperature_dew_point_capability = true;
  frame = await clickSidebar(page, 'Climate — Moisture & psychrometrics', 'Humidity and psychrometrics'); text = await bodyText(frame); if (!text.includes('Station pressure') || !text.includes('published monthly mean is available')) throw new Error('Measured monthly station-pressure capability missing.'); report.checks.measured_monthly_station_pressure = true;
  if (report.page_errors.length) throw new Error(`Browser page error(s): ${report.page_errors.join(' | ')}`); report.success = true;
} catch (error) { report.error = String(error?.stack || error); process.exitCode = 1; }
finally { report.completed_at_utc = new Date().toISOString(); await fs.mkdir('artifacts/pr83-contract-smoke', { recursive: true }); await fs.writeFile(OUT_PATH, JSON.stringify(report, null, 2)); console.log(JSON.stringify(report, null, 2)); if (browser) await browser.close().catch(() => {}); }
