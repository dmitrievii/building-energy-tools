...
async function emitComponentClick(frame, tooltip) {
  await frame.evaluate(async ({ tooltip }) => {
    if (!window.Streamlit || !window.__GLOBAL_DATA__) throw new Error('Streamlit component globals unavailable');
    window.__GLOBAL_DATA__.last_object_clicked_tooltip = tooltip;
    window.__GLOBAL_DATA__.last_object_clicked_count += 1;
    await new Promise((r) => setTimeout(r, 250));
    window.__GLOBAL_DATA__.previous_data = { last_object_clicked_tooltip: tooltip };
    window.Streamlit.setComponentValue({ last_object_clicked_tooltip: tooltip });
  }, { tooltip });
}
...
async function sample(context, i) {
  ...
  try {
    ...
    await emitComponentClick(opened.frame, chosen.content);
...
const result = {
  schema: 'climate-0.6.2-live-map-performance-v4',
...
console.log('LIVE_MAP_PERFORMANCE_0_6_2_V4=' + JSON.stringify(result));
