...
class PageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "html":
            ...
            self.h1_count += 1
        elif tag == "main" and values.get("id"):
            self.main_ids.add(values["id"] or "")
        elif tag == "a" and values.get("href"):
            self.hrefs.append(values["href"] or "")
        elif tag == "link" and values.get("href"):
            self.asset_urls.append(values["href"] or "")
        elif tag in {"img", "source", "iframe"} and values.get("src"):
        ...
        elif tag == "nav" and values.get("aria-label") == "Primary navigation":
...
def _resolve_local(page: Path, href: str) -> Path | None:
    ...
    try:
        ...
        raise AssertionError(f"local link escapes site root: {page.relative_to(SITE_ROOT)} -> {href}")
...
def validate_site() -> list[str]:
    ...
    for relative in sorted(EXPECTED_PAGES & actual_pages):
        ...
        if "main" not in parser.main_ids:
            errors.append(f"{relative}: missing <main id=\"main\">")
        if not "".join(parser.title_text).strip():
            errors.append(f"{relative}: missing document title")
        if not parser.has_viewport:
            errors.append(f"{relative}: missing responsive viewport meta")
        ...
        for asset in parser.asset_urls:
            if _is_external(asset):
                errors.append(f"{relative}: external runtime asset is not allowed: {asset}")
                continue
            try:
                target = _resolve_local(page, asset)
            except AssertionError as exc:
                errors.append(str(exc))
                continue
            if target is not None and not target.exists():
                errors.append(f"{relative}: missing local asset {asset}")
        ...
        for href in parser.hrefs:
            ...
            if _is_external(href):
                ...
                if host not in ALLOWED_EXTERNAL_LINK_HOSTS:
                    errors.append(f"{relative}: unreviewed external link host {host}: {href}")
                continue
            try:
    ...
    if climate_url not in index or climate_url not in tools:
        errors.append("Climate Analyzer must be directly linked from Home and Tools")
    if "not an official TU Graz" not in index:
        errors.append("Home must retain the project independence notice")
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"Static site validation PASS: {len(EXPECTED_PAGES)} pages, zero runtime dependencies")
    return 0
